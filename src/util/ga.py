...
class Generator:
    ...
    def generate_random_pitch(self) -> Note:
        ...
        pitch = random.choice(pitch_base)
        return pitch
...
""" def save_melody(midi : MIDIFile, file_name: str):
    with open(file_name + "midi", "wb") as output_file:
    ...
    FluidSynth(
        sound_font=FONT
    ).midi_to_audio(file_name+".midi", file_name + ".wav")
 """
def fitness_fun(chromosome: Chromosome) -> Fitness:
    
    #chromossome_to_melody(chromosome, "teste")
...
def selection(population_fitness: Population_Fitness) -> Population:
    ...
    population using one of the following operators per element:
        ...
        3. Duplication
        4. Inversion
    Each operator has a probability of being selected.  
    """
